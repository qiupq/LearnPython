###############################################################################
# The rexec module is disabled as of Python 2.3, with no current replacement
###############################################################################
...
class Guard(rexec.RExec):
    def r_open(self, name, mode='r', bufsz=-1):
        abspath = os.path.abspath(name)                    # expand '..', etc.
        absdir, absfile = os.path.split(abspath)           # get directory name
        if absdir != SafeDir:
